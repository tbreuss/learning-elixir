defmodule Structs do
  ...
  koan "Use the put_in macro to replace a nested value" do
    ...
    assert put_in(airline.plane.maker, :airbus) == %Airline{plane: %Plane{maker: :airbus}, name: "Southwest"}
  end

  koan "Use the update_in macro to modify a nested value" do
    airline = %Airline{plane: %Plane{maker: :boeing, passengers: 200}}
    assert update_in(airline.plane.passengers, fn x -> x + 2 end) == %Airline{name: "Southwest", plane: %Plane{maker: :boeing, passengers: 202}}
  end
